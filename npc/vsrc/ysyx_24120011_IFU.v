    ...
    if(rst) begin
        ...
        if(state == ysyx_24120011_IFU_IDLE) begin
            ...
            if(pc[31:24] == 8'ha0) begin
                araddr <= pc + (((arlen +1) - 1) - arlen_cnt)*4;
            end
            else begin
                araddr <= pc;
            end
            ...
            araddr <= 'd0;
            if(rvalid && (arlen_cnt == (arlen +1))) begin
                inst <= M0_rdata;
            end
            else begin
            end
    ...
    if(rst) begin
        ...
        if(state == ysyx_24120011_IFU_IDLE) begin
            ...
            if(rvalid  && rready) begin
                ...
                icache[index][31+((arlen +1) - 1 - (arlen_cnt-1))*32 -: 32] <= M0_rdata;
            end
            else begin
            end
        end
...
always@(*)begin
    case(state)
        ...
        ysyx_24120011_IFU_AXI_RDATA: next_state = (rvalid  && rready )     ? (M0_rlast ? ysyx_24120011_IFU_IDLE : ysyx_24120011_IFU_AXI_RADDR) : ysyx_24120011_IFU_AXI_RDATA;
